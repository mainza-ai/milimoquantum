...
class TestWebSocketConnection:
    ...
    async def test_websocket_connection(self):
        ...
        uri = f"{WEBSOCKET_URL}/test_client"
        try:
    ...
    async def test_websocket_ping_pong(self):
        ...
        uri = f"{WEBSOCKET_URL}/test_client_ping"
        try:
    ...
    async def test_websocket_broadcast(self):
        ...
        uri = f"{WEBSOCKET_URL}/test_client_broadcast"
        try:
            async with websockets.connect(uri, close_timeout=10) as ws:
            ...
            pytest.skip(f"WebSocket broadcast failed: {e}")
...
class TestWebSocketJobSync:
    ...
    async def test_job_subscription(self):
        ...
        uri = f"{WEBSOCKET_URL}/job_client_1"
        try:
            async with websockets.connect(uri, close_timeout=10) as ws:
                await ws.send(json.dumps({
                    "type": "subscribe_job",
            ...
            pytest.skip(f"Job subscription failed: {e}")

    async def test_multiple_subscriptions(self):
        """Test multiple job subscriptions."""
        uri = f"{WEBSOCKET_URL}/job_client_multi"
        try:
            async with websockets.connect(uri, close_timeout=10) as ws:
            ...
            pytest.skip(f"Multiple subscriptions failed: {e}")
...
class TestWebSocketReconnection:
    ...
    async def test_connection_close_and_reconnect(self):
        """Test connection close and reconnect."""
        uri = f"{WEBSOCKET_URL}/reconnect_client"
        try:
            async with websockets.connect(uri, close_timeout=5) as ws:
                await ws.send(json.dumps({"type": "ping"}))
                response = await asyncio.wait_for(ws.recv(), timeout=5.0)
...
class TestWebSocketErrors:
    ...
    async def test_invalid_message_format(self):
        ...
        uri = f"{WEBSOCKET_URL}/error_client"
        try:
            async with websockets.connect(uri, close_timeout=5) as ws:
            ...
            pytest.skip(f"Invalid message test failed: {e}")

    async def test_unknown_message_type(self):
        """Test handling unknown message type."""
        uri = f"{WEBSOCKET_URL}/unknown_client"
        try:
            async with websockets.connect(uri, close_timeout=5) as ws:
                await ws.send(json.dumps({"type": "unknown_type"}))
        except Exception as e:
            pytest.skip(f"Unknown type test failed: {e}")
